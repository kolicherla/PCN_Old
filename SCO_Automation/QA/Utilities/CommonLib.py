...
class CommonFunctions():
    ...
    def readExcelData(self):
        ...
        # TestDataLoc = os.path.join(os.path.dirname(os.getcwd())) +"/TestData/"+strFileName.replace('/',"\"")
        ...
        strData = pd.read_csv(TestDataLoc)
        dfData = pd.DataFrame(strData)
        # print(dfData)
        ...
        for i in range(0, rowcount):
            ...
            if strTCName == TCname:
                ...
                flag = False
        if flag != True:
            print("Could not find the test case Name")

        for col in range(0, colcount):
            name = dfData.iloc[i - 1, col]
            value = dfData.iloc[i, col]
            data_dic[name] = value
        return data_dic
